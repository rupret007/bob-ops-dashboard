...
- Verify generated HTML starts with `data-snapshot-trust="saved"`, readable named sections, and a visible timestamp/read-only explanation. With JavaScript off or navigation initialization interrupted, decision controls stay disabled and no composer links appear. Successful navigation restores compact home; only the existing accepted-snapshot checks can set trust to `current`. The stale-state smoke must still prove poll failure and >45-minute refresh silence switch it to a last-verified snapshot with one bounded **Retry now** action.
- Verify the first-screen next action names one real decision or red/yellow project, opens its existing panel, and focuses the exact sanitized row. The glance name is the work; the place line is an allowlisted Decide / Owner hold / Red / Review / CI wait plus an existing type for project rows. Review and CI wait must not share leftover look copy. A malformed or absent target must never select a neighbor, and a soft paint must preserve the selected tab.
- Verify first-screen type tabs are live types plus Parked. Leftover-only types sit under Parked and must not look like active agents or a Jeff yes.
- Test mixed pending/work snapshots: ordinary pending decisions keep priority; red and review yellow beat CI running/pending yellow; in-flight CI still beats only the explicit standing owner kinds. Parked must still open the exact valid owner hold, including a collapsed low-risk row, without reviewing or authorizing it. Verify first-paint / soft-paint parity and no invented destination when the hold or Decisions panel is absent.
