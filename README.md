...
The board follows every inherited product lane. GitHub-backed rows use live repository, default-branch CI, and open-PR state for WebJam, Turdanoid, Show Night, StoryBoard, Andrea NanoBot, StoryLiner, Bob Ops Dashboard, RadDadSite, Rad Dad Merch, and Cursor-OpenClaw Integration. OpenClaw Runtime is explicitly **Local-only** because no authoritative remote can be claimed; StoryDesk now has a private GitHub remote and stays high-level on this board; private GitHub lanes stay **high-level only** and are not public live-repo, CI, or PR tap rows; Bob the Bot is a distinct private application lane shown **high-level only** and is not a public live-repo, CI, or PR tap row; private-media work appears only as a high-level **Owner-only** boundary because upload and publishing stay outside this board.
...
- [Mini local proof and Bob coordination — 2026-08-26](docs/handoffs/2026-08-26-mini-local-proof.md) records the later exact-head proof, non-overlapping drafts, local runner findings, and remaining gates.
...
Hosted PR validation runs `python3 qa-offline.py` using existing runner tools; no dependency or browser installation is triggered. Optional local `--browser` adds Chromium at 320/390/1280px: full visible glance/title text, exact-row focus and no writes, popups or external requests. It requires an available Playwright/Chromium installation. Where test-dependency setup is authorized, use `npm ci --ignore-scripts` and `npx --no-install playwright install chromium`. The pinned npm dependency is test-only, not used by the public page. Local browser evidence and hosted claim-smoke evidence must be reported separately. See the [offline hosted integration handoff](docs/handoffs/2026-09-09-offline-hosted-integration.md).
