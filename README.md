...
- Phone-first board: one **pulse strip** (freshness + agents) + one short status + a **tab per existing type** (Live, Apps, Cisco, Bob, Media, Parked; Decisions when something needs a yes). First screen is not the project wall. Live still holds the music stack (Vault, StoryBoard, Show Night, WebJam). **Abilities** are a collapsed footer. Engineer notes stay behind collapsed **How this board works**.
