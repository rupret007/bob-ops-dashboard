...
CHIP_COLORS = {
  ...
  "parked": ("#64748b", "#0f172a", "#cbd5e1"),
  "jeff-gate": ("#7c3aed", "#2e1065", "#e9d5ff"),
}
...
<style>
  :root {{ --bg:#0b1220; --panel:#121a2b; --panel2:#182235; --text:#e8eefc; --muted:#9aa8c7; --border:#243049; --accent:#38bdf8; --link:#7dd3fc; }}
  * {{ box-sizing:border-box; }}
  html,body {{ margin:0; padding:0; background:var(--bg); color:var(--text); font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif; line-height:1.45; }}
  a {{ color:var(--link); text-decoration:none; }} a:hover {{ text-decoration:underline; }}
  .wrap {{ max-width:1100px; margin:0 auto; padding:1.25rem 1rem 3rem; }}
  header.hero {{ background:linear-gradient(145deg,#0f172a 0%,#1e293b 55%,#0b3b4a 100%); border:1px solid var(--border); border-radius:16px; padding:1.25rem 1.35rem; margin-bottom:1.25rem; }}
  ...
  .chip {{ display:inline-flex; align-items:center; gap:.35rem; border:1px solid var(--c); background:var(--bg); color:var(--fg); border-radius:999px; padding:.15rem .6rem; font-size:.72rem; font-weight:700; text-transform:uppercase; white-space:nowrap; }}
  ...
  nav.toc a {{ background:var(--panel); border:1px solid var(--border); color:var(--text); padding:.4rem .7rem; border-radius:999px; font-size:.82rem; }}
  ...
  .card {{ background:var(--panel); border:1px solid var(--border); border-radius:14px; padding:.9rem 1rem; display:flex; flex-direction:column; gap:.35rem; min-height:140px; }}
  ...
  .notes {{ margin:.25rem 0 0; color:#d6def0; font-size:.88rem; }}
  ...
  .banner {{ background:#172554; border:1px solid #1d4ed8; color:#bfdbfe; border-radius:12px; padding:.75rem 1rem; margin-bottom:1rem; font-size:.88rem; }}
</style>
</head>
<body>
<div class="wrap">
  <header class="hero">
    <h1>Bob Ops Dashboard</h1>
  ...
  <div class="banner">Public status page — no secrets, tokens, CSOne customer paths, or private handoff text. AdoptIQ is high-level Cisco CS desktop summary only.</div>
  <nav class="toc">
