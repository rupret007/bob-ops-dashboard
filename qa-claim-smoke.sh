...
grep -q 'Privacy/visibility owner hold' "$REFRESH" || fail "refresh.sh missing AI Music Vault privacy hold"
if grep -q 'Private docs/index' "$REFRESH"; then
  fail "refresh.sh still falsely claims AI Music Vault is private"
fi
...
if "Private docs/index" in html or "Private docs/index" in str(st):
    raise SystemExit("generated board falsely claims AI Music Vault is private")
if "Privacy/visibility owner hold" not in html or "Privacy/visibility owner hold" not in str(st):
    raise SystemExit("generated board missing AI Music Vault privacy hold")
